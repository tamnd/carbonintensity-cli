// Package cli assembles the carbonintensity command tree from the carbonintensity
...
// NewApp assembles the kit application from the carbonintensity domain. The
...
//
// To add a command, declare it in carbonintensity/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := carbonintensity.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(carbonintensity.Domain{}).Register(app)
