...
describes what users get. Pick a format with `-o`, or let carbonintensity choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
carbonintensity <command> -o table     # a rounded, color-aware grid for reading
carbonintensity <command> -o markdown  # a GitHub pipe table to paste into docs (alias: md)
carbonintensity <command> -o jsonl     # one JSON object per line, for piping
carbonintensity <command> -o json      # a single JSON array
carbonintensity <command> -o csv       # spreadsheet friendly
carbonintensity <command> -o tsv       # tab-separated
carbonintensity <command> -o url       # just the URL column
carbonintensity <command> -o raw       # the underlying bytes, unformatted
```
...
```bash
carbonintensity <command> --fields id,title,url
```
...
```bash
carbonintensity <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
carbonintensity <command>            # a table, because this is a terminal
carbonintensity <command> | wc -l    # JSONL, because this is a pipe
```
